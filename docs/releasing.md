...
Version **0.1.5** was published on GitHub on 24 September 2026; it passes Enter combinations through to agents and follows 0.1.4, which added drag and drop of vault paths, persistent sessions, automatic shell restart and restored output. The last completed Community review below applies to 0.1.2; the directory detects new releases automatically and queues its scan.
...
6. Obsidian periodically detects published releases and queues a review; no new submission is needed. To trigger detection immediately, use **More actions → Check for new releases** in the Community dashboard. A source push or unpublished draft alone does not publish an update.
